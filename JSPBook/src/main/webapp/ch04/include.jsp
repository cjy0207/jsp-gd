...
<body>
	...
	<h3>정적 include</h3>
	<h4>JSP 파일이 서블릿 코드로 변환되기 전에 그대로 복사되어 붙혀넣기 됨(즉, 하나의 JSP처럼 동작)</h4>
	<h4>변수/선언문/스크립틀릿 등 모두 공유 가능</h4>
	<%@ include file="include_date.jsp" %>
	<%= test %>
	
	<%-- 정리 : 둘다 여러 페이지를 한 페이지 안에 담는 기능
		<%@ include %>는 정적 포함 - JSP파일 내용(소스 코드)을 그대로 복붙해서 하나의 JSP로 컴파일
		<jsp:include />는 동적 포함 - JSP를 별도로 실행하고 그 결과(HTML)를 삽입
	 --%>
